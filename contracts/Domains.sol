...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld)
        payable
        ERC721("Table Name Service", "TABLA")
    {
    ...
    function price(string calldata domain) public pure returns (uint256) {
        uint256 len = StringUtils.strlen(domain);
        require(len > 0, "Domain must be >1 character long");
        if (len == 3) {
    ...
    function register(string calldata domain) public payable {
        if (domains[domain] != address(0)) revert AlreadyRegistered();
        if (!valid(domain)) revert InvalidName(domain);
        uint256 _price = this.price(domain);
        require(msg.value >= _price, "Not enough Matic to register");
    ...
    function getAddress(string calldata domain) public view returns (address) {
    ...
    function getRecord(string calldata record)
        public
        view
